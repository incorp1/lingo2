...
test("непрозрачные color-mix() в CSS имеют фолбэк для Safari 15", () => {
  ...
  for (const file of readdirSync(cssDir).filter(f => f.endsWith(".css"))) {
    ...
    for (const m of css.matchAll(decl)) {
      ...
      // Внутри @supports (color: color-mix(...)) фолбэк не нужен — Safari 15 блок пропускает.
      const lastSupports = css.lastIndexOf("@supports (color: color-mix", m.index);
      if (lastSupports !== -1 && css.slice(lastSupports, m.index).split("{").length - css.slice(lastSupports, m.index).split("}").length > 0) continue;
