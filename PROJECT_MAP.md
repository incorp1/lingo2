...
4. В `tests/card-editor-variant-c.test.js`, `tests/deck-browser-variant-a.test.js` и `tests/difficulty-cache.test.js` есть жёсткие ожидания текущей версии (**3.20.0**), включая regex с экранированными точками. При новом релизе обнови именно ожидания версии, не удаляя проверки. Общую согласованность также проверяет `tests/p46-p60-regression.test.js`.
